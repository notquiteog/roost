...
class WebSearchTool(Tool):
    ...
    async def _brave(self, query: str, count: int) -> list[dict]:
        if not self.api_key:
            raise ToolError('the brave backend needs ROOST_SEARCH_KEY')
    ...
    async def _tavily(self, query: str, count: int) -> list[dict]:
        if not self.api_key:
            raise ToolError('the tavily backend needs ROOST_SEARCH_KEY')
        body = await self._json(
    ...
    async def _searxng(self, query: str, count: int) -> list[dict]:
        ...
        if not self.base_url:
            raise ToolError('the searxng backend needs ROOST_SEARCH_URL')
        body = await self._json(
    ...
    async def _duckduckgo(self, query: str, count: int) -> list[dict]:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.post(
                ...
                data={'q': query},
                headers={'User-Agent': 'Mozilla/5.0 (compatible; Roost/0.1)'},
            ) as resp:
                if resp.status != 200:
        ...
        if 'anomaly' in markup.lower() or 'result__a' not in markup:
            raise ToolError(
                ...
                'ROOST_SEARCH_BACKEND to brave or tavily with ROOST_SEARCH_KEY, or to '
                'searxng with ROOST_SEARCH_URL pointing at your own instance.'
            )
