...
TOKENS: dict[str, Param] = {
    ...
    # An input picture, for image-to-video and img2img templates. What goes
    # into the graph is a *filename* — the one ComfyUI gives back after the
    # bytes are uploaded — because a LoadImage node takes a name from the
    # server's own input directory and not an image. The provider does that
    # upload; this only has to say that the control exists.
    'image': Param('image', 'Starting image', 'image', group='prompt',
                   help='For a template built around LoadImage: an image-to-video or img2img graph. '
                        'The file is uploaded to ComfyUI and the graph is given its name.'),
    'mask': Param('mask', 'Mask', 'image', group='prompt', advanced=True,
                  help='For an inpainting template. White is repainted, black is kept.'),
}
...
FALLBACKS.update({'prompt': '', 'negative': '', 'checkpoint': '', 'image': '', 'mask': ''})
...
def install(
    ...
    *,
    ...
    if overrides:
        path.with_suffix('.openmirror.json').write_text(json.dumps(overrides, indent=2))
...
def load(path: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    sidecar = path.with_suffix('.openmirror.json')
