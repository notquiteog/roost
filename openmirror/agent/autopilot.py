...
if TYPE_CHECKING:
    from openmirror.agent.stage import RawFrame
...
def encode_frame(
    source: bytes | RawFrame, *, width: int = FRAME_WIDTH, quality: int = FRAME_QUALITY
) -> bytes:
    ...
    Takes either a `RawFrame` — pixels as the grabber returned them, which is
    what the stream hands over — or PNG bytes, which is what a caller holding
    a screenshot already has. The raw path is the one that matters: it is the
    whole reason the capture side stopped compressing frames nobody was going
    to store.

    BICUBIC, not LANCZOS. This is a halving downscale that is then thrown
    through a lossy JPEG at quality 55; measured here the two encode to files
    within three percent of each other and LANCZOS costs a third more time on
    the one loop in this project that runs several times a second.
    """
    ...
    if isinstance(source, bytes | bytearray | memoryview):
        ...
        image = Image.frombytes(source.mode, source.size, source.data)
...
def _grab(stage) -> bytes | RawFrame:
    """Raw pixels where the stage can give them, PNG where it cannot.

    Kept tolerant on purpose: `capture_raw` is an addition to the stage
    protocol, and something that only implements `capture` — an older stage, a
    fake in a test — should stream rather than fail.
    """
    grab_raw = getattr(stage, 'capture_raw', None)
    return grab_raw() if callable(grab_raw) else stage.capture()
...
async def frames(stage, *, interval: float = FRAME_INTERVAL) -> AsyncIterator[bytes]:
    ...
    while True:
        ...
        try:
            frame = await asyncio.to_thread(_grab, stage)
            yield await asyncio.to_thread(encode_frame, frame)
            consecutive_failures = 0
