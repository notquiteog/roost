...
@dataclass(frozen=True, slots=True)
class RawFrame:
    """Pixels as the grabber handed them over, before anything encodes them.

    The streaming path wants one JPEG per frame and nothing else. Handing it a
    PNG meant every frame paid a full compress and a full decompress purely to
    arrive back at the bytes `mss` had already returned — the only CPU-bound
    loop in the project, several times a second, for as long as someone was
    watching. So capture can hand over the buffer instead.

    `data` is packed RGB, three bytes a pixel, rows top to bottom: what
    `Image.frombytes('RGB', frame.size, frame.data)` expects.
    """
...
class Stage(abc.ABC):
    ...
    def capture_raw(self) -> RawFrame:
        """The same picture, as raw pixels rather than PNG bytes.

        This is what the frame stream asks for, because it only ever wants a
        JPEG at the end and the PNG in between is pure cost. The default is
        honest but slow — encode a PNG and decode it again — so that a stage
        which has no cheaper path, or a fake in a test, keeps working
        unchanged. Stages whose grabber already returns pixels override it and
        skip the round trip; see `XStage`.
        """
        import io

        from PIL import Image

        with Image.open(io.BytesIO(self.capture())) as im:
            rgb = im.convert('RGB')
            return RawFrame(rgb.tobytes(), rgb.width, rgb.height)
...
class XStage(Stage):
    ...
    def capture_raw(self) -> RawFrame:
        """Straight from `mss`, which already hands back packed RGB.

        Nothing here encodes anything. `capture()` is this plus a PNG, for the
        callers that want a file; the frame stream calls this one and pays for
        a JPEG only.
        """
        try:
            import mss
        except ImportError as exc:
            raise StageUnavailable(f'screen capture needs mss: {exc}') from exc

        r = self._region
        with mss.MSS(display=self.display) as sct:
            raw = sct.grab({'left': r.x, 'top': r.y, 'width': r.width, 'height': r.height})
        width, height = raw.size
        return RawFrame(raw.rgb, width, height)
    ...
    def capture(self) -> bytes:
        try:
            ...
            raise StageUnavailable(f'screen capture needs pillow: {exc}') from exc
    ...
    def __init__(self, width: int = 1920, height: int = 1080, *, depth: int = 24) -> None:
        if not shutil.which('Xvfb'):
            raise StageUnavailable(
                ...
                'Install it (apt install xvfb) — or set OPENMIRROR_DESKTOP_STAGE=shared to let it '
        ...
        self._xauth = Path(tempfile.mkdtemp(prefix='openmirror-stage-')) / 'Xauthority'
...
class WaylandStage(Stage):
    ...
    def __init__(self, region: Rect | None = None, *, output: str = '') -> None:
        from openmirror.agent import desktop as desktop_mod
    ...
    def capture_raw(self) -> RawFrame:
        """Decoded once and cropped, rather than re-encoded in between.

        The compositor's screenshot tool only speaks PNG, so one decode is
        unavoidable here. What is avoidable is `capture()`'s re-encode of the
        crop followed by the base class decoding it straight back again.
        """
        import io

        from PIL import Image

        with Image.open(io.BytesIO(self._desktop.capture(output=self.output))) as im:
            image = im.convert('RGB')
            region = self._region
            if region is not None and not (region.x == 0 and region.y == 0):
                image = image.crop(
...
def pick_monitor(wanted: str, display: str = '') -> Monitor | None:
    ...
    if not found:
        raise StageUnavailable(
            ...
            'could not be enumerated. Install xrandr, or clear OPENMIRROR_DESKTOP_MONITOR.'
        )
    if wanted.isdigit():
        index = int(wanted)
...
def shared_stage(cfg) -> Stage:
    ...
    if not display:
        raise StageUnavailable(
            'there is no X display to drive. Set DISPLAY, or OPENMIRROR_DESKTOP_STAGE=virtual '
