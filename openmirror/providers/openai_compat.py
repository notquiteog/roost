...
class OpenAICompatProvider(ChatProvider, EmbeddingProvider, STTProvider, TTSProvider, ImageProvider):
    ...
    async def generate(
        ...
        # An image to work from turns this into a different endpoint. `/edits`
        # is the whole of image-to-image on this API — there is no strength and
        # no init-image field on `/generations` — and without it gpt-image-1
        # here is text-to-image only, which is most of what it is not for.
        source = kw.pop('image', None)
        if isinstance(source, bytes | bytearray) or isinstance(source, list):
            return await self._edit(prompt, model, n, size, source, kw, progress)
    ...
    async def _edit(
        self, prompt: str, model: str, n: int, size: str, source: Any,
        kw: dict[str, Any], progress: Any = None,
    ) -> list[GeneratedMedia]:
        """Image-to-image, which on this API is a multipart request to /edits.

        Several images may be sent at once to gpt-image-1 and it composes
        them — a product and a room, and a prompt saying to put one in the
        other. That is worth supporting properly rather than taking only the
        first: it is the single thing this model does that the diffusion
        backends in this build cannot.
        """
        import base64

        blobs = [b for b in (source if isinstance(source, list) else [source])
                 if isinstance(b, bytes | bytearray)]
        if not blobs:
            raise RuntimeError(f'{self.provider_id}: an edit needs at least one image')
        ...
        for index, blob in enumerate(blobs):
            # `image[]` for several, `image` for one. gpt-image-1 accepts the
            # bracketed form either way; dall-e-2's edit endpoint does not.
            field = 'image[]' if len(blobs) > 1 else 'image'
            form.add_field(field, bytes(blob), filename=f'source-{index}.png', content_type='image/png')
        mask = kw.pop('mask', None)
        if isinstance(mask, bytes | bytearray):
            form.add_field('mask', bytes(mask), filename='mask.png', content_type='image/png')
        for key in ('quality', 'background', 'output_format', 'output_compression', 'input_fidelity'):
            value = kw.get(key)
            if value not in (None, '', 'auto'):
                form.add_field(key, str(value))

        if progress:
            progress(None, f'editing on {self.provider_id}')

        headers = {k: v for k, v in self._headers().items() if k.lower() != 'content-type'}
        async with self._session() as session:
            async with session.post(
                f'{self.base_url}/images/edits', data=form, headers=headers
            ) as resp:
                if resp.status != 200:
                    raise RuntimeError(
                        f'{self.provider_id} image edit: HTTP {resp.status}: {(await resp.text())[:300]}'
                    )
                body = await resp.json()

        fmt = kw.get('output_format', 'png')
        return [
            GeneratedMedia(
                data=base64.b64decode(row['b64_json']),
                media_type=f'image/{fmt}',
        ...
        if model.startswith('gpt-image'):
            return params + [
                ...
                Param('image', 'Images to work from', 'image', group='prompt',
                      help='Supplying one switches to editing. Several can be given at once and '
                           'gpt-image-1 composes them — a product and a room, and a prompt saying '
                           'to put one into the other.'),
                Param('mask', 'Mask', 'image', group='prompt', advanced=True,
                      help='Only used with an image. The transparent area is what gets repainted.'),
                Param('input_fidelity', 'Input fidelity', 'enum', default='low', options=['low', 'high'],
                      group='quality', advanced=True,
                      help='High keeps faces and logos from the source image recognisable through '
                           'the edit. It costs more.'),
            ]
