...
`openmirror.media.workflow` is what makes it true: a template marks its inputs with
...
class ComfyUIProvider(VideoProvider):
    ...
    def _template_path(self, model: str) -> Path:
        ...
        if WORKFLOW_DIR.resolve() not in path.parents or not path.is_file():
            available = sorted(p.stem for p in WORKFLOW_DIR.glob('*.json') if not p.name.endswith('.openmirror.json'))
            raise FileNotFoundError(
                ...
                'openmirror/providers/workflows/README.md.'
            )
        return path
    ...
    async def generate(
        self, prompt: str, *, model: str, progress: Any = None, n: int = 1, size: str = '', **kw: Any
    ) -> list[GeneratedMedia]:
        import random
        ...
        # A picture has to exist on the ComfyUI box before a graph can name
        # it, so any image-shaped value is uploaded and replaced by the
        # filename that comes back. Done here rather than in the workflow
        # module because it is the one part of this that needs the network.
        for field in ('image', 'mask'):
            blob = kw.get(field)
            if isinstance(blob, bytes | bytearray):
                kw[field] = await self._upload(bytes(blob), field)
        ...
        async with self.transport.session() as session:
            ...
            outputs = await self._await_completion(session, prompt_id, progress)
            return await self._collect(session, outputs, int(seed))

    async def _await_completion(
        self, session: aiohttp.ClientSession, prompt_id: str, progress: Any = None
    ) -> dict[str, Any]:
        ...
        Coarse, but not nothing: `/queue` says how many jobs are ahead of this
        one, and "second in the queue" is a true and useful thing to say while
        a shared GPU works through somebody else's render. What is never said
        is a percentage, because nothing here knows one.
        """
        ...
        while asyncio.get_running_loop().time() < deadline:
            ...
            if progress:
                progress(None, await self._queue_note(session, prompt_id))
            await asyncio.sleep(self.poll_interval)
        raise TimeoutError(f'{self.provider_id}: job {prompt_id} did not finish within {self.timeout}s')

    async def _queue_note(self, session: aiohttp.ClientSession, prompt_id: str) -> str:
        """Where this job is in the line, in words."""
        try:
            async with session.get(f'{self.base_url}/queue', headers=self._headers()) as resp:
                if resp.status != 200:
                    return 'rendering'
                queue = await resp.json()
        except Exception:  # noqa: BLE001 - a missing queue endpoint is not a failed job
            return 'rendering'

        running = [row for row in (queue.get('queue_running') or [])]
        if any(prompt_id in str(row) for row in running):
            return 'rendering on the GPU'
        pending = queue.get('queue_pending') or []
        for index, row in enumerate(pending):
            if prompt_id in str(row):
                return 'next in the queue' if index == 0 else f'number {index + 1} in the queue'
        return 'rendering'

    async def _upload(self, data: bytes, field: str) -> str:
        """Put a picture where a LoadImage node can find it.

        ComfyUI's `/upload/image` writes into the server's input directory and
        returns the name it stored it under, which is what the graph needs.
        The name it returns is not always the name it was sent — a collision
        gets a suffix — so the answer is used rather than the request.
        """
        form = aiohttp.FormData()
        form.add_field('image', data, filename=f'openmirror-{uuid.uuid4().hex[:8]}-{field}.png',
        ...
        async with self.transport.session(120) as session:
            async with session.post(f'{self.base_url}/upload/image', data=form, headers=headers) as resp:
                if resp.status != 200:
                    raise RuntimeError(
                        f'{self.provider_id}: could not upload the input image: HTTP {resp.status}'
                    )
    ...
    async def models(self) -> list[dict[str, Any]]:
        ...
        for path in sorted(WORKFLOW_DIR.glob('*.json')):
            if path.name.endswith('.openmirror.json'):
                continue
            try:
